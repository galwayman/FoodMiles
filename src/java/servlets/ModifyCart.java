...
public class ModifyCart extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String s = request.getParameter("foodItemID");
        int fID = Integer.parseInt(request.getParameter("foodItemID"));
        FoodDAO fA = new FoodDAO();
        
        String direction = "shoppingCart";

        FoodItem selectedFood = fA.getSCItem(s);

        List<FoodCountry> foodCountry = fA.getFoodCountryStart(fID);

        HttpSession session = request.getSession();
        session.setAttribute("foodCountry", foodCountry);
        session.setAttribute("selectFIDirection", direction);

        session.setAttribute("f", selectedFood);
